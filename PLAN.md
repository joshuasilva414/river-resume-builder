...
V1.2 is implemented and deployed to staging under the [approved plan](docs/implementation/v1.2-plan.md). Post-migration recovery and the field-picker follow-up passed. Hosted PDF/DOCX evidence, paginated review, HTML import, matching, wording, Contact preview, PDF export, scoring, and Trash checks passed. Final template-schema and JavaScript-retrieval corrections are being prepared for staging. Its composable schemas, simple evidence, URL imports, successful-score allowances, and one-save template workflow supersede the conflicting historical V1 policies below. Track delivery in [v1.2 status](docs/implementation/v1.2.md), [hosted acceptance](docs/implementation/v1.2-hosted-acceptance.md), and [Paper designs](docs/implementation/v1.2-paper.md). This release target is staging only; do not promote it to main or production.
