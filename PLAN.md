...
V1.2.1 is being implemented and validated for staging under the [delivery record](docs/implementation/v1.2.1.md). That record tracks Paper fidelity, complete-resume comparisons, automatic built-in upgrades, local preview, refinement repair and hosted acceptance. No production promotion is authorized.

V1.2 is complete, deployed and verified on staging under the [approved plan](docs/implementation/v1.2-plan.md). Automated, local browser and representative hosted checks passed, including live PDF/DOCX evidence, complete-set review, HTML/JavaScript import, matching/wording, composable templates, contextual previews, export, scoring and Trash. The final populated isolated restore passed 82 tables, 242 retained objects, 58 citations, six exports and 12 template fixtures. See [v1.2 status](docs/implementation/v1.2.md), [hosted acceptance](docs/implementation/v1.2-hosted-acceptance.md), [recovery evidence](docs/implementation/v1.2-staging-recovery.md) and [Paper designs](docs/implementation/v1.2-paper.md). The v1.2 contract supersedes conflicting historical V1 policies below. This release is staging only; do not promote it to main or production.
...
- Opening River or its logo goes to Job targets. The shared header omits the redundant Personal workspace label. Document runtime lives at `/runtime` for staging/local development only, without the Phase 0 badge; production hides its navigation and rejects diagnostic calls. Paper references: production Job targets `NG-0`, staging/local runtime `AGI-0`.
