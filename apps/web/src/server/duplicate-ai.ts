  ...
  Effect.gen(function* () {
    ...
    return yield* attempt(() =>
      store.startDuplicateAi(
        actor,
        input,
        env.DUPLICATE_AI_WORKFLOW ? duplicateAiProfile(env) : null,
      ),
    );
  });
...
export const inspectDuplicateAi = (env: Env, id: string) =>
  Effect.gen(function* () {
    ...
    return { ...value, configured: Boolean(env.DUPLICATE_AI_WORKFLOW && duplicateAiProfile(env)) };
  });
