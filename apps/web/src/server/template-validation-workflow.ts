...
import { CUSTOM_RENDERER_VERSION, graphInventory, templateFixtures } from "@river/templates";
          ...
          await step.do(
            ...
            async () => {
              ...
              await repository.updateOperation(operationId, {
                state: "Running",
                stage: `Validating ${index + 1} of ${templateFixtures.length}: ${fixture.name}`,
              });
              ...
              const first = Schema.decodeUnknownSync(CompiledResult)(
                await this.env.DOCUMENTS.run(job),
              );
              ...
              const second = Schema.decodeUnknownSync(CompiledResult)(
                await this.env.DOCUMENTS.run(job),
              );
              const issues = [
