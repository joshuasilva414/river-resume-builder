...
To reproduce the web dry run, build with `CLOUDFLARE_ENV=production pnpm --filter @river/web build`, then run `pnpm exec wrangler deploy --config dist/server/wrangler.json --dry-run` from `apps/web`. For the document service, run `pnpm exec wrangler deploy --env production --dry-run --containers-rollout=none` from `apps/documents`. Both web environments share the local output directory: rebuild with the intended environment and inspect the generated account/name/bindings before any actual deployment.
