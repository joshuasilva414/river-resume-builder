...
The personal GitHub **River** registration form is now prepared with those exact values. Wildcard redirects and device flow remain disabled; expiring user access tokens retain the default enabled setting. **Register application has not been submitted.** Production credential creation/installation and publication remain pending the separate approval. The final staging acceptance backup/restore passes 148 objects and four exports; see `recovery.md`.
