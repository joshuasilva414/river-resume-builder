...
Updated 2026-09-05. All commands target the personal River account. Staging and production use separate pinned D1/R2 resources; ACM UTSA is unsupported. Production is published, and its first scheduled backup plus isolated restore pass. See `production.md`.
...
## Final staging acceptance restore — 2026-09-05

The snapshot `backups/database/staging/2026-09-06T01-50-08.082Z-45748b89-0c7c-4533-b393-b523ac8a7fce/snapshot.json.gz` is retained with SHA-256 `9f8a3ae0dacca1cd5dd513c0fbde83cc62309ddd2e61dd3812a95213d28a459d`. It restores into a new unpublished local SQLite database with 72 base tables, exact FTS equality, valid foreign keys, 148 retained objects, 24 exact citations and four exported checkpoints. Report: `test-results/recovery/drill-e7d40656-da2f-41c7-b1cc-b763331c424c/report.json`.

The supplemental `final-acceptance-proof.json` compares this database with the previous isolated restore. All 45 original library items and revisions, 23 evidence revisions, four original checkpoints, three old exports, four original draft revision/data pairs and four earlier qualification reports remain byte-for-byte unchanged. All nine canonical documents/digests are preserved. The new complete Minimal responses match their raw SHA-256 values and contain all six results with complete input coverage. The timed USAA export retains its exact digest and 29 acknowledgments. The original DOCX SHA-256 also remains unchanged.

The first manual snapshot command failed before export at a Wrangler D1 query. Its private output stayed suppressed. A read-only `SELECT 1` succeeded, then one explicit retry completed. The underlying transient failure remains unconfirmed. No live database was overwritten, no Workflow redispatched and no attempt counter reset. Logs: `/tmp/river-final-acceptance-backup.log`, `/tmp/river-final-acceptance-backup-retry.log`, `/tmp/river-final-acceptance-restore.log`.


## First scheduled production backup — 2026-09-05

The published production cron created UTC date `2026-09-06` with Operation `01a074cd-5d40-70f2-b262-785b5593743a`. It succeeded on application attempt 1 at `2026-09-06T03:40:20.659Z`. The retained v2 manifest is `backups/database/production/2026-09-06-01a074cd-5d40-70f2-b262-785b5593743a/manifest.json`, SHA-256 `858f84e83201ff8c7d62973d4c610ad0358aa24a797750a8e7d35f2db04ce0c9`. This proves the deployed dedicated token, exact production D1 export endpoint, Workflow dispatch and private R2 finalization.

The fresh isolated restore verifies the manifest and SQL hashes, all 72 table counts, database integrity, foreign keys and exact FTS equality. It retains the one verified Owner and first-attempt backup identity. The snapshot includes the initial fictional test records but has zero retained document objects, citations or exports; production document acceptance remains separate. No live data was replaced and no restored Operation was dispatched. Report: `test-results/recovery/drill-d7ba9a43-c3fc-4188-8f45-86a4b3f73f02/report.json`. The receipt and complete private restore files are preserved in the main workspace's ignored recovery directory. Logs: `/tmp/river-production-daily-{status,receipt,restore}.log`.


## Final production export restore — 2026-09-05

Migration `0028_library_archival.sql` was applied additively after separate private staging/production backups. The final production snapshot is `backups/database/production/2026-09-06T04-12-08.821Z-379ff4c1-7be8-4095-af6e-d4737492875b/snapshot.json.gz`, SHA-256 `29153be59f8dbac1507dfb10615ddf4e964251a4db27ab3ca2785e5aefc61c7e`. It includes all 29 migrations and 72 base tables. Its isolated restore passes integrity, foreign keys, exact FTS equality, all table counts, four retained document hashes and the original exported checkpoint. Report: `test-results/recovery/drill-a821db33-aa86-4c43-ae01-dc94e14d4cee/report.json`.

Comparison with the pre-migration snapshot verifies all three fictional library items are archived with unchanged immutable revisions. The original checkpoint and export record are unchanged, including the approved acknowledgment and four artifact identities. The fictional job is archived. Private comparison proof: `test-results/production-acceptance/archival-proof.json`. No live database was overwritten, no restored Workflow was dispatched, and unrelated production records were preserved. This closes final-domain recovery acceptance.

## v1.1 encrypted provider connections

Migrations 0031/0032 add account connections, preferences, and AI execution metadata. Database snapshots contain encrypted keys. Recovery requires the matching `AI_CREDENTIAL_ENCRYPTION_KEY`, retained separately from database/object backups. Do not replace that secret without a re-encryption plan. See [v1.1 cutover](v1.1-cutover.md) for drain, staged acceptance, retention, and rollback requirements.

The final v1.1 production drill restored 76 base tables, one exact citation, two exported checkpoints and 28 retained files. The restored personal OpenAI connection decrypted with the retained production secret and loaded its model catalog. See [production recovery evidence](v1.1-production.md#recovery-and-preservation).


## v1.2 recovery compatibility — 2026-09-08

Migrations 0034–0038 add automatic template approval state, optional source extraction settings and Trash state, URL job imports, scoring policies/usage/reservations, and template Trash state. Composable schema definitions and structured values use existing retained JSON columns. Both manual and scheduled backups discover all base tables and bundle the exact applied migration SQL, so these fields require no hard-coded table inventory. Source originals and extraction objects remain required after a source moves to Trash.

A complete v1.2 restore must use the exported `scoring_policy` row, including its configured limit and revision. Migration 0037 creates a default row while recreating the schema. The isolated restore now removes that local seed before replaying the complete snapshot and rejects a v1.2 snapshot missing the policy. It does not recalculate usage from retained scoring results: administrator resets, per-account overrides, and Reserved/Consumed/Released state must survive exactly. Only the operation admission trigger is suspended during history replay; foreign keys, search maintenance, and quota settlement/release triggers remain active. Admission is restored before commit.

Migration filenames include digits in the v1.2 suffix. Manual backup validation and recovery fixture discovery now accept names such as `0037_v1_2_usage.sql`; the prior letters-only suffix rule rejected these files. Scheduled backup migration discovery already uses the complete SQL glob.

Focused local proof round-trips every base-table row with current migrations, including encrypted connection ciphertext, source extraction selection, optional evidence sources/types, structured entry/schema JSON, template approval/Trash state, job import text/analysis, policy overrides, reset usage, and pending/settled reservations. It verifies that one newly retained result charges once after restore, repeated settlement is free, and cancellation releases remaining reservations. A pre-v1.2 snapshot also restores with its own schema and then migrates additively, preserving retained sample responses while seeding usage and reserving unfinished samples. Foreign keys and FTS equality are checked in every restore.

Before the staging migration, retain a fresh staging backup and receipt with the existing encryption secret preserved separately. After the new web Worker is deployed, capture a fresh backup and run an isolated restore drill through 0038. Confirm exact policy/usage/reservation rows as well as retained document objects, template/schema JSON, source objects in Trash, and URL imports. Leave restored Workflow dispatch disabled. A database restore must not reset quotas or redispatch old provider submissions. The matching `AI_CREDENTIAL_ENCRYPTION_KEY` remains required; this local fixture check proves ciphertext preservation, not live decryption.

Validation: three focused recovery tests and four read-only deployment configuration tests pass (`node --test scripts/lib/recovery.test.mjs scripts/lib/deployment.test.mjs`). Focused Biome checks pass. Logs are `/tmp/river-recovery-final.log` and `/tmp/river-recovery-lint.log`.

Local configuration checks confirm the staging D1/R2 backup pins and document-service binding agree, and environment/branch guards reject cross-environment bundles. No live Cloudflare state, hosted backup, migration, or restoration was performed by this compatibility check. The parent v1.2 delivery record must supply fresh staging deployment and recovery evidence.
