...
The Owner supplied both credentials and explicitly approved installing them on the personal staging Worker. Token access to the pinned D1 database and OpenAI model was verified without exposing values. Hosted task `01a071e7-75f6-7cef-af70-0b5fd368708c` generated a real 11-point candidate and completed its first preview attempt through the deployed Container. Graph, text completeness/multiplicity/order and runtime checks passed. Complete component changes, the one-page PDF, extracted text and report were reviewed in the browser. Acceptance saved Draft revision `01a071e9-8a06-76d5-901f-142ad1344a8c` without changing Classic. Hosted validation `01a071ea-7ef6-77ab-9430-db189dac8e5b` rendered all four fixtures twice and passed. This graph is Validated and still awaits separate Owner visual approval. Hosted conversation inspection confirmed the saved original brief, selected accepted base and prior-instruction defaults. Daily backup and isolated restore also passed; see `recovery.md`.


## Hosted staging visual approval — 2026-09-05

Following the Owner's authorization for staging UI test approvals, all four retained fixtures of “Synthetic hosted 11-point proof” were visually inspected, including both long-content pages, repeated wording and Unicode. Their reports pass with matching repeated-render identities. The normal visual acknowledgment and approval command advanced exact revision `01a071e9-8a06-76d5-901f-142ad1344a8c` from Validated to Approved. Its graph and digest are unchanged. This is a staging lifecycle test and grants no ATS qualification.

The separate source-promotion journey now saves an independent Draft from a generic 3pt-to-2pt paragraph-spacing adjustment. Full source and personal content are excluded from that model input. See `source-refinement.md` for task, proposal and immutable revision identities. Approval of the earlier graph does not approve this new Draft.
