...
Browser inspection covered both complete source files, all five intended fields, unchanged extracted text, both real PDFs and the complete passing report with no warnings. Desktop dark launch/review screenshots were checked. The actual preview URL returns 401 without authentication. Automatic approval review rejected recording the Owner checkbox attestation for test acceptance; explicit user approval is pending. The candidate remains Pending and no publication was started. Browser publication/export/structured-return and narrow viewport verification are not yet claimed. Focused D1 tests prove branch isolation, exact retry identity and preservation of source-only wording outside the structured tree.
