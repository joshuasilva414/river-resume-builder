...
Deployed to staging through migration `0025_checkpoint_labels.sql`. Paper boards 70–71 define capture, chronology and restoration; see `history-scoring-design.md`.
...
- Desktop history was inspected in both themes with visible focus. At this capture milestone, complete comparison and hosted checks were still open; subsequent verification is recorded below. Mobile history/restoration and the real Owner tailoring session remain open.

## Pinned content and PDF comparison

Paper boards 71–72 have a deployed read-only implementation. Each side selects an exact checkpoint or saved working-draft revision. Loading saves the full response in the open comparison, with a SHA-256 identity. Lightweight draft-head polling reports newer saved work or previews without replacing the selected content or artifact. Explicit loading is required to refresh. Stale revision requests fail; incomplete saved drafts can still be compared without successful compilation. Inputs are bounded at 2 MB, with no truncation.
...
Web commit `cc5370e` is deployed as `f307e560-c38e-4256-822f-ef73e4e0c830`. A retained backup preceded additive migrations 0019–0025. All 123 Workers tests across 23 files, workspace types/lint, the staging bundle and the complete offline document fixtures passed before deployment.

Hosted restoration copied synthetic checkpoint `01a07091-d743-76c2-a7ee-51294410467a` into branch `01a072c5-ae2a-7b48-b1a3-900b94ff1b7c`. Its real r0 preview succeeded. Labeled checkpoint `01a072c6-9255-7d8f-bc16-f44d904b5c6b` retained “Synthetic hosted restoration baseline” and rendered successfully. Chronology identifies its exact origin and distinguishes the original export from this unexported saved checkpoint.

Hosted comparison pins the complete checkpoint and draft inputs independently. Both actual PDFs rendered; the base at 75% did not change the compare pane's fit-width zoom. The working-draft Scores tab stays disabled. Desktop comparison light layout and chronology dark layout were screenshot-reviewed. Advancing the synthetic branch to r1 preserved its labeled r0 checkpoint and original export. No new evidence acknowledgment, source-review attestation or export was submitted. Mobile review and accepted-source comparison remain open.
